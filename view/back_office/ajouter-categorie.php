...
$controllerCategorie = new CategorieEvenementC();
echo "Contrôleur créé.<br>";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($_POST['nom_categorie'])) {
        $nom_categorie_err = "Veuillez entrer un nom de catégorie.";
    } else {
        $nom_categorie = $_POST['nom_categorie'];
        echo "Nom de catégorie : $nom_categorie.<br>";
        if (!preg_match("/^[a-zA-ZÀ-ÿ ]*$/",$nom_categorie)) {
            $nom_categorie_err = "Seules les lettres et les espaces blancs sont autorisés dans le nom de la catégorie.";
        }
    }

    if (empty($nom_categorie_err)) {
        echo "Pas d'erreur dans le nom de la catégorie.<br>";

        // Vérifier si la catégorie existe déjà
        $categorieExistante = $controllerCategorie->searchCategorie($nom_categorie);
        if ($categorieExistante == false) {
            echo "Catégorie non existante, ajout en cours.<br>";

            // Obtenir le dernier ID utilisé
            $lastId = $controllerCategorie->getLastId();
            $newId = $lastId + 1;
            echo "Nouvel ID de catégorie : $newId.<br>";

            // Créer une instance de la classe CategorieEvenement
            $categorie = new CategorieEvenement($newId, $nom_categorie);

            // Appeler la méthode addCategorie
            try {
                $controllerCategorie->addCategorie($categorie);
                echo "La catégorie a été ajoutée avec succès.";
            } catch (Exception $e) {
                echo 'Erreur: ' . $e->getMessage();
            }
        } else {
          $nom_categorie_err = "La catégorie existe déjà.";
        }
    }
}
?>
    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item"><a href="categorie.php">Les categories </a></li>
                    ...
                    <div class="col-lg-12 col-12">
                        ...
                        <form method="POST" enctype="multipart/form-data">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="nom_categorie" name="nom_categorie" placeholder="Entrez le nom de la catégorie" required>
                                <!-- Afficher le message d'erreur ici -->
                                <small class="form-text text-danger"><?php echo $nom_categorie_err; ?></small>
                            </div>
