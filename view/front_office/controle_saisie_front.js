...
const id_articleInput = document.getElementById("id_article");
...
function validerId_article() {
    const id_articleValeur = id_articleInput.value;
    const id_articleRegex = /^[0-9]*$/;
    const erreurId_article = document.getElementById("erreurId_article");
  
    if (id_articleValeur === "") {
      erreurId_article.innerHTML = "<span style='color:red'>Veuillez remplir ce champ</span>";
    } else if (!id_articleValeur.match(id_articleRegex)) {
      erreurId_article.innerHTML = "<span style='color:red'>Veuillez entrer un id d'article valide (chiffres uniquement)</span>";
    } else {
      erreurId_article.innerHTML = "<span style='color:green'> Correct </span>";
    }
  }
